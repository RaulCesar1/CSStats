	...
	data: new SlashCommandBuilder()
		...
		.addBooleanOption((option) =>
			option
				.setName('escolha')
				.setDescription(
					'True para procurar perfil utilizando ID e False para utilizar URL customizada.'
				)
        .setRequired(true)
		)
		.addStringOption((option) =>
			option
				.setName('pesquisa')
				.setDescription('Digite a URL customizada ou ID')
        .setRequired(true)
		),
		...
		try {
			...
			function criar_embed(
				...
				let embed = new Discord.MessageEmbed()
					...
					.setAuthor({
						name:
							!Autor || Autor === 'undefined' ? 'Perfil inexistente!' : Autor,
					})
					.addFields([
						{
							name: 'Nacionalidade',
              ...
              ${
						...
						},
						{
							name: 'Level:',
							value: `${
								!level || level === 'undefined' ? 'Indefinido' : level
							}`,
						},
					])
					.setThumbnail(
			...
			if (boolean === true) {
				...
				const EpochTime =
					req2.data.response.players.player[0].timecreated * 1000
				const EpochTime2 =
					req2.data.response.players.player[0].lastlogoff * 1000
				const d2 = new Date(EpochTime2)
				const d = new Date(EpochTime)
				const dataCorreta = d.toLocaleString()
				const dataCorreta2 = d2.toLocaleString()
